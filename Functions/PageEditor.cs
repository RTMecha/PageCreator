...
namespace PageCreator.Functions
{
    ...
    {
		// TO DO
		// - Work on QuickElement editor mode. (once that's done, this should be ready to release)
		...
		public static void Init()
        {
            ...
            {
				Debug.LogWarningFormat("{0}PageEditor has already been initialized!", PagePlugin.className);
            }
        }
		...
		IEnumerator GenerateUI()
		{
			...
			Debug.LogFormat("{0}Canvas Scale Factor: {1}\nResoultion: {2}", PagePlugin.className, canvas.scaleFactor, new Vector2(Screen.width, Screen.height));
				...
				//Interface Editor
				{
					...
					//Times
					{
						...
						timesMinIF.onValueChanged.AddListener(delegate (string _val)
						{
                            ...
                            {
								Debug.LogFormat("{0}Setting Interface Times Min: {1}", PagePlugin.className, num);
						...
						timesMaxIF.onValueChanged.AddListener(delegate (string _val)
						{
							if (!loadingEditor && currentInterfaceSelection != null && float.TryParse(_val, out float num))
							{
								Debug.LogFormat("{0}Setting Interface Times Max: {1}", PagePlugin.className, num);
				...
				//Branch Editor
				{
					...
					//Clear Screen
					{
						...
						toggle.onValueChanged.AddListener(delegate (bool _val)
						{
							if (!loadingEditor && currentBranchSelection != null)
							{
								Debug.LogFormat("{0}Setting Interface Branch Clear Screen: {1}", PagePlugin.className, _val);
		...
		IEnumerator RefreshInterface()
		{
			...
			if (editMode == EditMode.Interface)
			{
				foreach (var inter in interfaces)
				{
					...
					{
						Debug.LogFormat("{0}Clicked {1}", PagePlugin.className, Path.GetFileName(inter.filePath));
					...
					// Add Branch
					{
						...
						if (RTFile.FileExists(RTFile.ApplicationDirectory + "BepInEx/plugins/Assets/add.png"))
							UIManager.GetImage(image, RTFile.ApplicationDirectory + "BepInEx/plugins/Assets/add.png");
					}
					...
					// Edit
					{
						...
						if (RTFile.FileExists(RTFile.ApplicationDirectory + "BepInEx/plugins/Assets/editor_gui_edit.png"))
							UIManager.GetImage(image, RTFile.ApplicationDirectory + "BepInEx/plugins/Assets/editor_gui_edit.png");
					}
					...
					if (inter.expanded)
					{
						...
						foreach (var branch in inter.branches)
						{
							...
							if (string.IsNullOrEmpty(searchTerm) || branch.name.ToLower().Contains(searchTerm.ToLower()))
							{
								...
								{
									Debug.LogFormat("{0}Clicked {1}", PagePlugin.className, branch.name);
								...
								foreach (var element in branch.elements)
								{
									if (expandedBranches.ContainsKey(inter.filePath + " - " + branch.name) && expandedBranches[inter.filePath + " - " + branch.name])
									{
										...
										{
											Debug.LogFormat("{0}Clicked {1}", PagePlugin.className, data);
			...
			if (editMode == EditMode.QuickElement)
            {
				foreach (var quickElement in QuickElementManager.AllQuickElements)
				{
					...
					if (!QuickElementManager.quickElements.ContainsKey(name))
					{
						baseButtonFunc.onClick.AddListener(delegate ()
						{
							Debug.LogFormat("{0}Clicked {1}", PagePlugin.className, name);
                ...
                case SelectionType.Interface:
                    {
                        if (currentInterfaceSelection != null)
                        {
                            ...
                            Debug.LogFormat("{0}Selected Interface: {1}", PagePlugin.className, Path.GetFileName(currentInterfaceSelection.filePath));
                        }
                        else
                        {
                ...
                case SelectionType.Branch:
                    {
                        if (currentBranchSelection != null)
                        {
                            ...
                            Debug.LogFormat("{0}Selected Branch: {1}", PagePlugin.className, currentBranchSelection.name);
						}
                        else
                        {
                            unselectedDialog.SetActive(true);
                ...
                case SelectionType.Element:
                    {
                        if (currentElementSelection != null && currentElementSelection.data != null && currentElementSelection.data.Count > 0)
                        {
							...
							Debug.LogFormat("{0}Selected Element: {1}", PagePlugin.className, currentElementSelection.data[0]);
                        }
                        else
                        {
                            unselectedDialog.SetActive(true);
				...
				case SelectionType.QuickElement:
                    {
						if (currentQuickElementSelection != null)
						{
							...
							Debug.LogFormat("{0}Selected QuickElement: {1}", PagePlugin.className, currentQuickElementSelection.name);
						}
						else
						{
							unselectedDialog.SetActive(true);
        ...
        void Exit()
        {
			Debug.LogFormat("{0}Exit Page Editor", PagePlugin.className);
            interfaces.Clear();
		...
		IEnumerator Save(Interface @interface)
		{
			...
			Debug.LogFormat("{0}Saving {1}", PagePlugin.className, inter.filePath.Replace(RTFile.ApplicationDirectory, ""));
		...
		IEnumerator Save()
        {
			Debug.LogFormat("{0}Saving...", PagePlugin.className);
			...
			Debug.LogFormat("{0}Saved!", PagePlugin.className);
			yield break;
		}
